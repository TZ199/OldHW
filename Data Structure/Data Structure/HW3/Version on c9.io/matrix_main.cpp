...
void StudentTest(){
	...
	Matrix m3(4,3,3);
	double result = 0;
	if(!m2.get(4,0,result))
	{
		std::cout << "false" << std::endl;
	}
	//Test for get
	std::cout << "test of get: " << result << std::endl;
	m2.set(0,0,5);
	m2.set(1,2,3);
	m2.set(2,3,7);
	m2.get(0,0,result);
	//Test for set
	std::cout << "test of set: " << result << std::endl;
	std::cout << m2 << std::endl;
	//Test for muti
	m2.multiply_by_coefficient(2);
	std::cout << m2;
	//Test for swap rows
	m2.swap_row(0,1);
	std::cout <<"after swap" << m2;
	...
	std::cout << "LR: " << std::endl << ma1[3] << std::endl;
	delete [] ma1;
	//Test for 0 rows or 0 columns when create a matrix
	Matrix s1(0,4,2);
	Matrix s2;
	assert(s1 == s2);
	Matrix s3(4,0,2);
	...
	assert(s4 == s6);
	//Add
	assert(s4.subtract(s6));
	assert(s4 == s7);
	
	//For corner case, the dimmenisions are different.
	assert(!s4.add(s1));
...
void ExtraCreditTest(){
	...
	//For a bigger target size 
	e1.resize(5,5,6);
	std::cout << e1 << std::endl;
	//For a smaller target size
	e1.resize(2,2,4);
	std::cout << e1 << std::endl;
	Matrix e2(2,2,2);
	assert(e2 == e1);
