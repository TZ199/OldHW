...
int countPaths(int row, int col, vector<vector<bool> >& blocked_grid)
{
  int counter = 0;
  if (row<0||col<0)
      return 0;
  if(blocked_grid[row][col])
    return 0;
  if (row==0&&col==0)
    return 1;
  else
    {
    counter += countPaths(row-1, col, blocked_grid) + countPaths(row, col-1,blocked_grid);
    return counter;
    }
  
}

void PrintPath(int x, int y, vector<vector<bool> > blocked_grid)
{
  if (countPaths(x, y,blocked_grid)==0)
    return;
  else 
  {
    cout << "(" << x << ", " << y << ")" << endl;
    if (countPaths(x,y-1,blocked_grid)>0)
      PrintPath(x, y-1, blocked_grid);
    else
      PrintPath(x-1, y, blocked_grid);
  }
}
...
int main(int argc, char* argv[]) {
  ...
  cout << "Number of paths from start to end: " << countPaths(start_x, start_y, blocked_grid) << endl;
   PrintPath(start_x, start_y, blocked_grid);
