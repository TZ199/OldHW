...
//Arguments: players in the vector that stores all information about players, aname is the name we want to check
// champion is the champion that player played.
...
//Arguments: champions in the vector that stores all information about champions, aname is the name of champion we want to check
//check = 1 means that champion is winning, check = 0 means that champion is losing, apeople is the player palys the champion in one match.
...
int main(int argc, char* argv[])
{
    ...
    if(option == "players" || option == "custom")
    {
                        ...
                        if(e2 != "minion")
                        {
                            ...
                            for(int j = 0; j<players.size(); j++)
                            {
                                if(players[j].getName() == e2 )
                                {
                                    ...
                                    players[j].addKill();
                                    int staple = atoi(e1.c_str());
                                    if (staple >= 1200000)
                                    {
                                        players[j].addLatekill();
                                    }
        ...
        //In custom part, we'll print out every palyer with their late kill (after 20min)
        //The sort priority is late kill > kill > death > name.
        else
        {
            sort(players.begin(), players.end(), sortHelper2);
        	//Cout the resluts to file.
            out_put << std::setw(23) << std::left << "PLAYER NAME" << std::setw(5) << std::right << "KILLS" <<
            std::setw(8) << std::right <<"DEATHS" << std::setw(14) << std::right <<"LATEKILLS" <<
            std::setw(26) << std::right <<"PLAYED WITH CHAMPION(S)" <<std::endl;
            for(int w = 0; w < players.size(); w++)
            {
                out_put << std::setw(23) << std::left << players[w].getName() << std::setw(5) << std::right << players[w].getKill() <<
                std::setw(8) << std::right <<players[w].getDeath() << std::setw(14) << std::right <<std::setprecision(2) << std::fixed <<
                players[w].getLatekill() <<"   "<< players[w].getChampion() <<std::endl;
            }
        }
