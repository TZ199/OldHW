...
template <class T> size_type remove_matching_elements(Vec<T>& v, const T& val)
{
  size_type temp = 0;
  
  for(typename Vec<T>::iterator i = v.begin(); i != v.end(); ++i)
  {
    cout << " D " << *i << endl; 
    if(*i == val)
    {
      cout << "DEBUG " << val << " "<< temp <<endl;
      typename Vec<T>::iterator j = i+1;
      if(j == v.end())
      {
        v.erase(i);
        break;
...
{
  cout << "m_alloc: " << m_alloc << ", m_size: " << m_size << "\n";
  cout << "m_data:\n";
