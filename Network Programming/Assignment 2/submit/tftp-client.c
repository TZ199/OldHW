 ...
 * all message types.
 */
static size_t tftp_enc_packet(tftp_t *tftp, const uint16_t opcode, uint16_t blkno, unsigned char *data, size_t datalen) {
 ...
 */
static int tftp_dec_opcode(unsigned char *buf, size_t buflen, uint16_t *opcode) {
  if (buflen < 2) {
    return 0xffff;
  }
...
static int tftp_dec_blkno(unsigned char *buf, size_t buflen, uint16_t *blkno) {
  if (buflen < 4) {
    return 0xffff;
  }
  *blkno = (buf[2] << 8) + buf[3];
...
static int tftp_dec_data(unsigned char *buf, size_t buflen, unsigned char **data, size_t *datalen) {
  if (buflen < 5) {
    *data = NULL;
    *datalen = 0;
    return 0xffff;
...
static int tftp_dec_error(unsigned char *buf, size_t buflen, uint16_t *errcode, char **msg) {
  int i;
  if (buflen < 5) {
    *msg = NULL;
    return 0xffff;
 ...
 */
static void tftp_socket(tftp_t *tftp) { 
 ...
 */
static void tftp_close(tftp_t *tftp) {
  assert(tftp);  
  (void) close(tftp->sd);
  (void) close(tftp->fd);
}
...
static int tftp_mainloop(tftp_t *tftp) {
  ...
  while (tftp->state != TFTP_STATE_CLOSED) {
    ...
    if (! timerisset(&tftp->timer) || timercmp(&now, &tftp->timer, >)) {
      ...
      int rc = getnameinfo((struct sockaddr *) &tftp->addr, tftp->addrlen, hoststr, sizeof(hoststr), portstr, sizeof(portstr), NI_NUMERICHOST | NI_NUMERICSERV);
      if (rc == 0) printf("Client: sento() %s port %s, ", hoststr, portstr);
    ...
    if (! FD_ISSET(tftp->sd, &fdset)) {
      retries--;
      if (! retries) {
        fprintf(stderr, "%s: timeout, aborting data transfer\n", progname);
        tftp_close(tftp);
        return EXIT_FAILURE;
      }
      continue;
    ...
    printf("Client: recvfrom(), %d bytes: %hi, %hi\n", buflen, ntohs(*(short*)buf), ntohs(*(short*)(buf+2)));
    if (buflen == -1) {
