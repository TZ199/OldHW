...
void state_standby(int *operation){
    ...
    printf("TFTP Server [PID %d]: block at recvfrom()\n", getpid());
    ...
    while(1){
        ...
        int rc = getnameinfo((struct sockaddr *) &ftCli, addrlen, hoststr, sizeof(hoststr), portstr, sizeof(portstr), NI_NUMERICHOST | NI_NUMERICSERV);
        if (rc == 0) printf("TFTP Server [PID %d]: recvfrom() %s port %s\n", getpid(), hoststr, portstr);
...
void state_wait(int *operation){
    ...
    printf("TFTP Server [PID %d]: recvfrom() %s port %d\n", getpid(), inet_ntoa(((struct sockaddr_in *)&ftCli)->sin_addr), ntohs(((struct sockaddr_in *)&ftCli)->sin_port));
