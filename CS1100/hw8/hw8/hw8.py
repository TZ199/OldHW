...
if __name__ == '__main__':
    ...
    while True:
        print "=============================="
        location = raw_input("Where (-1 to exit) ==> ")
        print location
        if location == '-1':
            break
        if location == '':
        ...
        for d in data:
            if d['city'].lower() == location.lower() or d['full_address'].lower().find(location.lower()) >= 0:
                ...
                if len(fliter_in) == 0 and len(fliter_not_in) == 0:
                    ...
                    if len(fliter_in) == 0 and len(fliter_not_in) > 0 and len(not_in) == 0:
                        for c in category:
                            categories[c] = categories.get(c,0) + 1
                        businesses.append(d)

                    if len(fliter_in) > 0 and len(fliter_not_in) > 0 and len(not_in) == 0 and sorted(is_in) == sorted(fliter_in):
                        for c in category:
                            categories[c] = categories.get(c,0) + 1
                        businesses.append(d)
        ...
        for category in sorted(categories):
            print('       %s (%d)') %(category,categories[category])
        ...
        if len(businesses) != 0:
            choice = raw_input('Print the businesses (y/n)? ==> ')
            print(choice)
            if choice == 'y':
